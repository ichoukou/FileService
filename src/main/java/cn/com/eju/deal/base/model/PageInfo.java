...
public class PageInfo {
	...
	public String getHtml() {
		...
		html.append("<div class=\"page-wrap clearfix\"><div  class=\"col-sm-4\"style=\"line-height: 30px;\"><p class=\"pagination-info\">共<span>"+dateCount+"</span>条记录</p></div><div  class=\"col-sm-8\"><ul class=\"pagination pull-right\" style=\"margin:0\">");
		...
		//前端省略号
		if((curpage - halfPageNum) >= halfPageNum && pageSize-1>ORDER_NUM){
		...
		for (int i = startPage; i <= endPage; i++) {
			if (i == curPage) {
				...
				html.append("<li><a href=\"javascript:void(0);\" name=\"sysPageBtn\" value=\"").append(i).append("\">").append(i).append("</a></li>");
			}
		}
				
		//后端省略号
		if((curpage + halfPageNum) < pageSize-1 && pageSize-1>ORDER_NUM){
			html.append(ellipsis);
		}
				
		if(curpage==pageSize){
			html.append("<li class=\"active\"><span>").append(pageSize).append("</span></li>");
			html.append("<li class=\"disabled\" ><a href=\"javascript:void(0);\" name=\"sysPageBtn\" value=\"").append(getNextPage()).append("\">下一页</a></li>");
		}else{
